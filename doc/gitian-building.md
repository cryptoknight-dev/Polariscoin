...
*Setup instructions for a gitian build of Polariscoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Polariscoin
...
- [Building Polariscoin](#building-polariscoin)
...
Clone the git repositories for polariscoin and gitian and then checkout the polariscoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the polariscoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building Polariscoin
----------------------

To build Polariscoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the polariscoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/polariscoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/polariscoin/polariscoin
...
For example:
```bash
URL=https://github.com/konez2k/polariscoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit polariscoin=${COMMIT} --url polariscoin=${URL} ../polariscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit polariscoin=${COMMIT} --url polariscoin=${URL} ../polariscoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit polariscoin=${COMMIT} --url polariscoin=${URL} ../polariscoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/polariscoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/polariscoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/polariscoin-build.assert
```
...
[polariscoin/gitian.sigs](https://github.com/polariscoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
