...
#if defined(NDEBUG)
#error "Polariscoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uPOL) are considered zero fee (for relaying and mining)
...
{
    RenameThread("polariscoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // POL: We use certain sporks during IBD, so check to see if they are
